...
void cComposableGenerator::InitFinishGens(cIniFile & a_IniFile)
{
	...
	for (AStringVector::const_iterator itr = Str.begin(); itr != Str.end(); ++itr)
	{
		...
		{
			LOGINFO("The POCPieces finisher is obsolete, you should use \"PieceStructures: POCPieces\" instead.");
			auto gen = std::make_shared<cPieceStructuresGen>(Seed);
			if (gen->Initialize("POCPieces", seaLevel, m_BiomeGen, m_CompositedHeightCache))
			{
				m_FinishGens.push_back(gen);
			}
		}
		...
		else if (NoCaseCompare(finisher, "TestRails") == 0)
		{
			LOGINFO("The TestRails finisher is obsolete, you should use \"PieceStructures: TestRails\" instead.");
			auto gen = std::make_shared<cPieceStructuresGen>(Seed);
			if (gen->Initialize("TestRails", seaLevel, m_BiomeGen, m_CompositedHeightCache))
			{
				m_FinishGens.push_back(gen);
			}
		}
